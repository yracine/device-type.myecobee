...
def authPage() {
    ...
    def redirectUrl = "${get_ST_URI_ROOT()}/oauth/initialize?appId=${app.id}&access_token=${state.accessToken}&apiServerUrl=${getServerUrl()}"
		...
		httpGet(deviceListParams) { resp ->
			...
			if (resp.status == 200) {
				...
				if (resp.status == 500 || resp.data.status.code == 14) {
					...
					refreshAuthToken(atomicState.refreshToken)
...
def refreshAuthToken(refreshToken) {
	log.debug "refreshAuthToken>about to refresh auth token"
	boolean result=false
    
	if(!refreshToken) {
		log.warn "refreshAuthToken>can not refresh OAuth token since there is no refreshToken stored"
	} else {
		...
		def refreshParams = [
				...
				uri   : "${get_URI_ROOT()}",
				path  : "/token",
				query : [grant_type: 'refresh_token', code: "${refreshToken}", client_id: stcid]
		]
...
def getAuthTokens() {
//	send("getAuthTokens> begin parent Tokens = ${atomicState}")
	if (isTokenExpired()) {
		refreshAuthToken()	
		log.debug "getAuthTokens>new tokens=${atomicState} after refreshAuthToken() call"
//		send "getAuthTokens>new tokens=${atomicState} after refreshAuthToken() call"
	}
	log.debug "getAuthTokens>end tokens=${atomicState}"
//	send "getAuthTokens>end tokens=${atomicState}"
