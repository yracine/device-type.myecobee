...
def presence(evt)
{
    ...
    if (evt.value == "present") {
        ...
        if (recentNotPresent) {
            log.debug "ecobeeResProg>skipping notification of arrival of ${person.displayName} because last departure was only ${now() - recentNotPresent.date.time} msec ago"
...
def presence(evt) {
    ...
    if (evt.value == "present") {
        ...
        if (recentNotPresent) {
            log.debug "ecobeeResProg>skipping notification of arrival of ${person.displayName} because last departure was only ${now() - recentNotPresent.date.time} msec ago"
            def message = "ResumeProg>${person.displayName} recently arrived"
            log.info message
            send(message)
        }           
	}
