...
//      just wait for the next cycle & do nothing for now.
    ...
    if (useDehumidifierAsHRVFlag =='true') {
        ...
        if (equipStatus.contains("dehumidifier")) {
            log.trace("dehumidifier should be running (${equipStatus}), time left to run = ${diffVentTimeInMin.toString()} within the current cycle")
            if (detailedNotif == 'true') {
        ...
        if (dehumidifierRunInMin < min_vent_time) {
            if (detailedNotif == 'true') {
                send "MonitorEcobeeHumidity>About to set dehumidifier on..."
            }
            ...
            delayInt = (delayInt > (1.5*60))? delayInt: (1.5*60) // Min. delay should be at least 1 minute, otherwise, the dehumidifier won't stop.
            if (detailedNotif == 'true') {
                send "MonitorEcobeeHumidity>need to run dehumidifier for ${diffVentTimeInMin.toString()} min. within the next hour,this cycle is for ${delayInt} min."
            }    
            runIn(delayInt, "turn_off_dehumidifier")  // turn off the dehumidifier after delay
        } else {
            if (detailedNotif == 'true') {
